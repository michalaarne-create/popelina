...
def _detect_marker_shape_cpu(pil_img, bbox: List[int]) -> Dict[str, object]:
    ...
    try:
        ...
        for candidate in (bin_inv, bin_nrm):
            ...
            for c in contours:
                ...
                if m_score > best_score:
                    best_score = m_score
                    best = {
                        "circle_like": circle_score,
                        "square_like": square_score,
                        "score": m_score,
                    }
        ...
        if hough_circle_found or best["circle_like"] >= best["square_like"]:
            ...
            out["marker_conf"] = round(max(conf, hough_conf if hough_circle_found else 0.0), 4)
        else:
