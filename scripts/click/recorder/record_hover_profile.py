...
Jak uĹĽywaÄ‡:
    (ai_cuda) python scripts/click/recorder/record_hover_profile.py

Instrukcja:
    - Ruszaj myszkÄ… po ekranie.
    - NaciĹ›nij ']' aby WĹÄ„CZYÄ† nagrywanie trajektorii.
    - NaciĹ›nij ']' ponownie aby JE WYĹÄ„CZYÄ† (zamyka segment).
    - Zatrzymaj skrypt Ctrl+C w konsoli, gdy nazbierasz kilka prĂłb.

Skrypt:
    - Zapisuje trajektorie i statystyki do data/hover_profile/profile.json
    - Na koĹ„cu wypisuje sugerowane:
        * docelowÄ… prÄ™dkoĹ›Ä‡ w px/s
        * minimalny dt miÄ™dzy punktami
"""
...
try:
    ...
    raise SystemExit(
        f"[record_hover_profile] ERROR: nie moĹĽna zaimportowaÄ‡ pynput ({exc}). "
        "Zainstaluj paczkÄ™ 'pynput' w envie ai_cuda."
    )
...
def main() -> None:
    ...
    print("Ruszaj myszkÄ… po ekranie.")
    print("Nagrywanie wĹ‚Ä…czasz/wyĹ‚Ä…czasz klawiszem ']'.")
    print("ZakoĹ„cz nagrywanie Ctrl+C.\n")
    ...
    if not segments:
        print("[record_hover_profile] Brak zarejestrowanych segmentĂłw â€” nic do profilu.")
        return
    ...
    print(f"  DEFAULT_SPEED_PX_PER_S â‰ {rec['DEFAULT_SPEED_PX_PER_S']:.1f}")
    print(f"  DEFAULT_MIN_DT â‰ {rec['DEFAULT_MIN_DT']:.4f} s")
