...
Usage (PowerShell):
  python scripts/click/fallback/send_random_behavior_path.py `
...
def ensure_control_agent(port: int) -> None:
    ...
    agent = root / "scripts" / "click" / "control_agent" / "control_agent.py"
    config = root / "scripts" / "click" / "control_agent" / "train.json"
    if not agent.exists():
