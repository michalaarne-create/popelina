...
Usage:
  python scripts/click/hover/run_auto_hover.py [--port 8765] [--speed-factor 1.0]
...
from scripts.click.hover.hover_bot import (
...
def _ensure_agent(port: int) -> subprocess.Popen:
    cfg = PROJECT_ROOT / "scripts" / "click" / "control_agent" / "train.json"
    agent = PROJECT_ROOT / "scripts" / "click" / "control_agent" / "control_agent.py"
    if not agent.exists():
