...
Usage (manual):
  python scripts/click/hover/send_hover_path.py --json data/screen/hover/hover_output_current/hover_output.json \
...
# DomyĹ›lne ustawienia â€žludzkiejâ€ť trajektorii hovera.
# UĹĽywane zarĂłwno przez to CLI, jak i przez main.py (HOVER_SPEED_DEFAULTS).
DEFAULT_SPEED_MODE = "normal"       # tryb prÄ™dkoĹ›ci w control_agent (_target_duration_ms)
DEFAULT_SPEED_FACTOR = 1.0          # globalny multiplicator prÄ™dkoĹ›ci segmentĂłw
DEFAULT_MIN_DT = 0.008              # minimalny odstÄ™p czasowy miÄ™dzy punktami (~125 Hz)
DEFAULT_SPEED_PX_PER_S = 740.0      # docelowa Ĺ›rednia prÄ™dkoĹ›Ä‡ kursora w px/s
# Gap boost: lekkie, ale wyczuwalne przyspieszenie na â€ždziurachâ€ť miÄ™dzy boxami.
# NiĹĽsza wartoĹ›Ä‡ => bardziej ludzko, mniej agresywne skoki.
DEFAULT_GAP_BOOST = 1.18            # ~18% szybciej w gapach zamiast 40% (1.4)
DEFAULT_LINE_JUMP_BOOST = 1.8       # miÄ™kkie przyspieszenie przy skokach miÄ™dzy liniami

# Katalog z profilem â€žprawdziwegoâ€ť hovera nagranym przez record_hover_profile.py.
PROFILE_PATH = Path(__file__).resolve().parents[2] / "data" / "hover_profile" / "profile.json"


def _apply_profile_overrides() -> None:
    """
    JeĹ›li istnieje data/hover_profile/profile.json (tworzone przez
    record_hover_profile.py), nadpisz domyĹ›lne parametry trajektorii tak,
    aby odpowiadaĹ‚y prawdziwym ruchom uĹĽytkownika.
    """
...
def _group_lines(seqs: List[Dict[str, Any]]) -> List[List[int]]:
    """
    Grupuj boxy w wiersze na podstawie Ĺ›redniej wysokoĹ›ci.
    Zwraca listÄ™ grup z indeksami sekwencji.
    """
...
def _is_spacing_like(seq: Dict[str, Any]) -> bool:
    """
    Heurystyka: czy box jest separatorem/odstÄ™pem a nie realnym tekstem.
    UĹĽywana, aby nie generowaÄ‡ punktĂłw tam, gdzie sÄ… tylko â€ždziuryâ€ť.
    """
    try:
...
def _split_columns(real_idx: List[int], seqs: List[Dict[str, Any]]) -> List[List[int]]:
    """
    Podziel sekwencje na pionowe kolumny (np. rĂłĹĽne panele/taby) na
    podstawie zakresĂłw X.
    """
    if not real_idx:
        return []
...
def _build_path_and_meta(
    ...
    *,
    ...
    """
    Zbuduj listÄ™ punktĂłw hovera oraz prostokÄ…ty â€ždziurâ€ť i indeksy skokĂłw
    miÄ™dzy liniami. To jest wolniejszy, ale czytelniejszy wariant do
    uĹĽycia z linii komend / debug.
    """
    ...
    # 4) Skalowanie odlegĹ‚oĹ›ci w poziomie (spacing_x).
