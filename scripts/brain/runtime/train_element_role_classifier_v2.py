...
def _build_manifest_content_hash(paths: Sequence[Path]) -> str:
    digester = hashlib.sha1()
    for path in sorted((Path(p).resolve() for p in paths), key=lambda item: str(item).lower()):
        try:
            payload = path.read_text(encoding="utf-8", errors="replace")
        except Exception:
            payload = ""
        digester.update(str(path.name).encode("utf-8", errors="ignore"))
        digester.update(b"\n")
        digester.update(payload.encode("utf-8", errors="ignore"))
        digester.update(b"\n--manifest-boundary--\n")
...
def build_rollout_payload(*, out_model: Path, payload: Dict[str, Any]) -> Dict[str, Any]:
    training_meta = payload.get("training_meta") if isinstance(payload.get("training_meta"), dict) else {}
    return {
        "release_family": "element_role_rollout",
        ...
        "dataset_hash": str(training_meta.get("dataset_hash") or ""),
        "metrics_holdout": training_meta.get("metrics_holdout") if isinstance(training_meta.get("metrics_holdout"), dict) else {},
        "created_at": str(training_meta.get("created_at") or ""),
    }
...
def main() -> int:
    ...
    parser.add_argument("--out-rollout", default="", help="Optional rollout contract JSON that points runtime to the trained model.")
    ...
    payload = {
        ...
        "training_meta": {
            ...
            "seed": int(args.seed),
            "dataset_hash": _build_manifest_content_hash(manifest_paths),
    ...
    if args.out_rollout:
        out_rollout = Path(args.out_rollout).resolve()
        out_rollout.parent.mkdir(parents=True, exist_ok=True)
        rollout_payload = build_rollout_payload(out_model=out_model, payload=payload)
        out_rollout.write_text(json.dumps(rollout_payload, ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"[TRAIN] rollout={out_rollout}")
    return 0
